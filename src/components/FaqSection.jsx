...
export default function FaqSection({ onQuote, onContact, onNavigate }) {
  ...
  const resultCount = Object.values(grouped).reduce((total, items) => total + items.length, 0);
  ...
  return (
    <section className={`faq-section faq-section--professional${searching ? ' is-searching' : ''}`} id="faq">
      <div className="page-shell help-hero">
        ...
        <div><label className="help-search"><Search aria-hidden="true" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search a question, product or term" aria-label="Search help topics" /></label><p className="help-search-status" role="status">{searching && `${resultCount} ${resultCount === 1 ? 'answer' : 'answers'} found. Results appear below.`}</p></div>
      </div>

      {!searching && <div className="page-shell help-paths" aria-label="Help topics">
      ...
      {!searching && <section className="page-shell help-essentials" aria-labelledby="help-essentials-title">
      ...
      <section className="page-shell help-faq" id="faq-list" aria-labelledby="faq-title">
        ...
        <div className="help-faq__groups">
          {Object.entries(grouped).map(([group, items]) => <section key={group}><h3>{group}</h3>{items.map(([question, answer], index) => <details key={question} open={searching || (group === 'Choosing Ford Protect' && index === 0)}><summary>{question}<span aria-hidden="true">+</span></summary><p>{answer}</p></details>)}</section>)}
